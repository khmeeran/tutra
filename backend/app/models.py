...
class CurriculumSource(Base, TimestampMixin):
...
class CurriculumImportJob(Base, TimestampMixin):
    __tablename__ = 'curriculum_import_jobs'
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    source_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('curriculum_sources.id', ondelete='CASCADE'))
    status: Mapped[str] = mapped_column(String(50))
    error_log: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

class CurriculumRawRecord(Base, TimestampMixin):
    __tablename__ = 'curriculum_raw_records'
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    job_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('curriculum_import_jobs.id', ondelete='CASCADE'))
    raw_text: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    raw_json: Mapped[Optional[Any]] = mapped_column(JSONB, nullable=True)
    inferred_grade: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    inferred_subject: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    validation_status: Mapped[str] = mapped_column(String(50), default='UNCHECKED')
